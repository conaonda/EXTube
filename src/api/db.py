...
_ALLOWED_UPDATE_FIELDS = {"status", "error", "result", "ply_path", "progress"}
...
class JobStore:
    ...
    def _migrate(self) -> None:
        ...
        if "progress" not in columns:
            self._conn.execute("ALTER TABLE jobs ADD COLUMN progress TEXT")
            self._conn.commit()
    ...
    def create(self, job_id: str, status: str, url: str) -> dict[str, Any]:
        ...
        sql = (
            "INSERT INTO jobs"
            " (id, status, url, error, result, ply_path, progress, created_at)"
            " VALUES (:id, :status, :url, :error, :result,"
            " :ply_path, :progress, :created_at)"
        )
        with self._lock:
